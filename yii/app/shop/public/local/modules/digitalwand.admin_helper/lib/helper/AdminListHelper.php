			...
			if ($this->hasDeleteRights()) {
				...
				foreach ($IDs as $id) {
					...
					$entityManager = new static::$entityManager(
						$model, empty($this->data) ? array() : $this->data, $id,
						$this
					);
		...
		if ($this->isPopup()) {
			...
			if ($this->hasDeleteRights()) {
				$actions['delete'] = array(
					...
					'ACTION' => "if(confirm('" . Loc::getMessage('DIGITALWAND_ADMIN_HELPER_LIST_DELETE_CONFIRM') . "')) " . $this->list->ActionDoGroup(
							$data[$this->pk()],
