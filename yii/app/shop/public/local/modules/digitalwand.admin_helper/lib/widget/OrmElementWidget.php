...
class OrmElementWidget extends NumberWidget
{
	...
	public function getValueReadonly()
	{
		...
		if (!empty($entityData)) {
			...
			/** @var \DigitalWand\AdminHelper\Helper\AdminBaseHelper $linkedHelper */
			$linkedHelper = $this->getSettings('HELPER');
			$module = $linkedHelper::getModule();
			$entity = $linkedHelper::getEntityCode();

			/* Конвертация camelCase в snake_case */
			$entityName = mb_strtolower(preg_replace(['/([a-z\d])([A-Z])/', '/([^_])([A-Z][a-z])/'], '$1_$2', $entity));

			return '[<a href="/bitrix/admin/admin_helper_route.php?lang=ru&module='.$module.'&view='.$entityName.'_edit&ID=' .
				$entityData['ID'] .
				'&entity='.$entity.'">' .
				$entityData['ID'] .
				'</a>] ' .
				static::prepareToOutput($entityName);
		}
