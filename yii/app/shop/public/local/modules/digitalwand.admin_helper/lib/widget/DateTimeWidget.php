...
class DateTimeWidget extends HelperWidget
{
	...
	protected function getEditHtml()
	{
		$value = $this->getValue();
		$converted = null;
		$type = $this->getSettings('TYPE') ?: 'FULL';
		...
		if (!empty($value)) {
			...
			$converted = ConvertTimeStamp($timestamp, $type);
		}

		return \CAdminCalendar::CalendarDate($this->getEditInputName(), $converted, "FULL", 10, true);
	}
	...
	public function processEditAction()
	{
		try {
			...
			if (!empty($value)) {
				$converted = new \Bitrix\Main\Type\Datetime($value);
			}
